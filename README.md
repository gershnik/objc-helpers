...
An ever-growing collection of utilities to make coding on Apple platforms in C++ or Objective-C++ more pleasant. Some functionality is also available on Linux.
...
- [What's included?](#whats-included)
    ...
    - [Boxing of any C++ objects in Objective-C ones](#boxing-of-any-c-objects-in-objective-c-ones)
    - [Comparators for Objective-C objects](#comparators-for-objective-c-objects)
    - [Printing Objective-C objects to C++ streams and std::format](#printing-objective-c-objects-to-c-streams-and-stdformat)
...
With the modern Clang compiler you can seamlessly convert C++ lambdas to blocks like this:
```c++
...
The `BlockUtil.h` header gives you the ability to solve all of these problems.

It provides two functions: `makeBlock` and `makeMutableBlock` that take any C++ callable as input and return an object
...
* If the callable is movable, it will be moved into the block, not copied. It will also be moved if the block is "copied to heap" 
  by the Objective-C runtime or `Block_copy` in plain C++.
...
dispatch_async(someQueue, makeBlock([ptr=std::move(ptr)]() {
    ptr->someMethod();
}));
...
In plain C++ there is no ARC magic, so you need to manually manage the block pointer's lifecycle using
...
DispatchTask<int> coro() {
    ...
    //this will execute asynchronously on the main queue
    ...
    //you can convert ObjC APIs with asynchronous callbacks to coroutines
...
DispatchTask<int> useGenerator() {
    ...
    //this will run generator asynchronously on the main queue
...
Sometimes you want to store a C++ object where an Objective-C object is expected. Perhaps there is
...
//This will raise an Objective-C exception if the type doesn't match
...
//since having inconsistent equality and hashing is one of the most common Objective-C errors
auto hash = obj1.hash;
...
### Printing Objective-C objects to C++ streams and std::format ###
...
Header `NSStringUtil.h` provides `NSStringCharAccess` - a fast accessor for `NSString` characters (as `char16_t`) via an STL container interface. This uses an approach similar to [`CFStringInlineBuffer`](https://developer.apple.com/documentation/corefoundation/cfstringinlinebuffer?language=objc). This facility can be used both from Objective-C++ and plain C++.
...
- Finally, produce a `"<full name of the type> object"` string.
